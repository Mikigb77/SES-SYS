<div class="MySection container">
    <div class="row">
        <div class="col-md-12" style="text-align: center;">
            <h1>Notícias</h1>
            <hr>
        </div>
        <div class="col-md-6" style="padding: 5%; padding-top:2.5%;">
            <div class="row">
                <div class="col-md-6" style="text-align: center;">
                    <h3>Construmat</h3>
                    <p>05/23</p>
                    <p>Feria de Barcelona</p>
                    <p>(del 23-25)<br><strong>Steel Embed System</strong><br>Stand A55</p>
                    <div>
                        <p>* Invitación <strong>GRATUITA</strong> disponible para profesionales:</p>
                        <a href="https://registration.firabarcelona.com/?cod_prom=WOTUNLQV#/es_ES/J025023/WEB" target="_blank">
                            <button type="button" class="btn btn-outline-dark"><strong>Invitacion Construmat</strong></button>
                        </a>
                    </div>

                </div>
                <div class="col-md-6">
                    <a href="https://registration.firabarcelona.com/?cod_prom=WOTUNLQV#/es_ES/J025023/WEB" target="_blank">
                        <img src="/media/images/Construmat-def.jpeg" alt="" style="width:100%;">
                    </a>
                </div>
            </div>
        </div>
        <div class="col-md-6" style="padding: 5%; padding-top:2.5%;">
            <div class="row">
                <div class="col-md-6" style="text-align: center;">
                    <h3>ACE</h3>
                    <p>9/22</p>
                    <p>Publicación en cuadrenos de estructuras.</p>
                    <p>Nº 75</p>
                    <div>
                        <p>* A partir de julio 2023 estará disponible en tienda desde aquí.</p>
                        <p>** Descuento para socios hasta el 80%.</p>
                        <a href="https://aceweb.cat/es/categoria-producto/cuaderno-de-estructuras/" target="_blank">
                            <button type="button" class="btn btn-outline-dark"><strong>Solicita la Revista</strong></button>
                        </a>
                    </div>

                </div>
                <div class="col-md-6">
                    <a href="https://aceweb.cat/es/categoria-producto/cuaderno-de-estructuras/" target="_blank">
                        <img src="/media/images/ACE-PortadaRev.jpg" alt="" style="width:100%;">
                    </a>
                </div>
            </div>
        </div>

        <div class="col-md-6" style="padding: 5%; padding-top:2.5%;">
            <div class="row">
                <div class="col-md-6" style="text-align: center;">
                    <h3>Rebuild 26</h3>
                    <p>5/22</p>
                    <p>Ifema Madrid</p>
                    <p><strong>SES, Steel Embed System</strong></p>
                    <div style="text-align: center;">
                        <br>
                        <a href="https://rebuild.ticketsnebext.com/rebuild2022/es/Products/Details/665243" target="_blank">
                            <button type="button" class="btn btn-outline-dark"><strong>Ver Detalles Expositor</strong></button>
                        </a>
                    </div>
                </div>
                <div class="col-md-6">
                    <iframe width="560" height="350" src="https://www.youtube.com/embed/xTBe5_u3AQU?start=1" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen style="width: 100%; border-radius:0%;"></iframe>

                </div>
            </div>
        </div>
        <div class="col-md-6" style="padding: 5%; padding-top:2.5%;">
            <div class="row">
                <div class="col-md-6" style="text-align: center;">
                    <h3>ACE</h3>
                    <p>05/22</p>
                    <p>Presentación del sistema SES en una sesión de formación ACE.</p>
                    <p><strong>SES Steel Embeed System</strong></p>
                    <p>Forjado preindustrializado</p>
                </div>
                <div class="col-md-6">
                    <iframe width="560" height="350" src="https://www.youtube.com/embed/tD7euT4iC3A?start=1" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen style="width: 100%; border-radius:0%;"></iframe>
                </div>
            </div>
        </div>
        <div class="col-md-6" style="padding: 5%; padding-top:2.5%;">
            <div class="row">
                <div class="col-md-6" style="text-align: center;">
                    <h3>E-ficiencia</h3>
                    <p>05/22</p>
                    <p><strong>SES, el nuevo sistema integrado de forjado que revoluciona la construcción</strong></p>

                    <div style="text-align: center;">
                        <br>
                        <a href="https://e-ficiencia.com/ses-nuevo-sistema-de-forjado-inteligente/" target="_blank">
                            <button type="button" class="btn btn-outline-dark"><strong>Ver Articulo</strong></button>
                        </a>
                    </div>
                </div>
                <div class="col-md-6" style="text-align: center;">
                    <img src="/media/images/Logos/ses-e-ficiencia.jpg" alt="E-ficiencia" style="width:100%;display: flex; justify-content: center; align-items: center;">
                </div>

            </div>
        </div>
    </div>
</div>